...
class GeminiCLIProcess(BaseCLIProcess):
    """Manages a Gemini CLI process with stdin/stdout communication."""

    def _get_default_cli_path(self) -> str:
        """Get the default Gemini CLI path."""
        base_dir = Path(__file__).parent.parent
        return str(base_dir / "gemini-cli" / "bundle" / "gemini.js")

    def _get_cli_executable_args(self) -> List[str]:
        """Get the executable and base arguments to start Gemini CLI."""
